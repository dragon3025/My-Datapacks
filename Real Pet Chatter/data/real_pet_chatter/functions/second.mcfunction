...
tag @e[type=#real_pet_chatter:noisy_mobs/friendly, tag=angry_pet] remove angry_pet
...
execute if score admin mobs_anger_cats matches 1.. as @e[type=#real_pet_chatter:noisy_mobs/friendly] at @s if entity @e[type=#hostile_status:neutral_or_hostile, tag=hostile, tag=!persistent, distance=..4] run tag @s add angry_pet
...
execute as @e[type=#real_pet_chatter:noisy_mobs/main, tag=chatting_mob_v2, name=!"silenced"] at @s run function real_pet_chatter:chatting_pet_second
