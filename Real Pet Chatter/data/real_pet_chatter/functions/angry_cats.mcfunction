...
execute as @s[nbt={Age:0}] if entity @e[tag=real_pet_chatter_hostile, distance=..4, nbt={PersistenceRequired:false}] if predicate real_pet_chatter:cat_yell_chance run playsound entity.cat.yell neutral @a ~ ~ ~ 2
...
execute as @s[nbt=!{Age:0}] if entity @e[tag=real_pet_chatter_hostile, distance=..4, nbt={PersistenceRequired:false}] if predicate real_pet_chatter:cat_yell_chance run playsound entity.cat.yell neutral @a ~ ~ ~ 2 2
