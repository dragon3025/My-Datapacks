tag @e[type=#real_pet_chatter:pets, distance=..1, limit=1, sort=nearest] add real_pet_chatter_viewed_pet
...
execute as @e[type=#real_pet_chatter:pets, tag=real_pet_chatter_viewed_pet, tag=chatting_mob_v2, tag=!angry_cat, name=!"silenced", nbt={Silent:true}] run data merge entity @s {Silent:false}
