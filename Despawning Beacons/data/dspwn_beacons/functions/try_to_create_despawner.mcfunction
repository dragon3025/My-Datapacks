...
execute positioned ~-50.5 ~-0.5 ~-50.5 if entity @e[type=#dspwn_beacons:hostile, type=!zombified_piglin, type=!hoglin, dx=100, dy=20, dz=100, nbt=!{PersistenceRequired:1b}] run tag @s add dspn_beacon_mob_in_range
...
execute as @s[tag=dspn_beacon_mob_in_range,tag=dspn_beacon_blocks_pass] positioned ~-50.5 ~-0.5 ~-50.5 run tag @e[type=#dspwn_beacons:hostile, type=!zombified_piglin, type=!hoglin, dx=100, dy=20, dz=100, nbt=!{PersistenceRequired:1b}, limit=1, sort=nearest] add dspn_beacon_blocking_mob
